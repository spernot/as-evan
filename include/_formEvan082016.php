...
if (isset($_POST['nom'])) {
	...
	if ($erreur) {
		...
		$destinataire = "ludovic.hoerdt@gmail.com";
// 		$destinataire = "sebastien67.pernot@yahoo.fr";
		...
		if (mail($destinataire, $sujet, $corps)) {
			...
			try{
				$fp = fopen ("evangelisation082016.csv", "a");
			...
			echo ("le mail d'inscription n'a pas été envoyé pour d'obscures raisons...<br/>Mais ne vous découragez pas, vous pouvez réessayer!");
		}
	}	
}
?>
...
<div>
	<table style="border: 0px;">
		...
		<tr>
			...
			<td><input type="radio" name="locomotion" value="Train" checked="checked"/>Train<br/>
				...
				<input type="radio" name="locomotion" value="A_la_nage" <?php echo( isset($_POST['locomotion']) && $locomotion == "A_la_nage" ? 'checked="checked"' : '') ?>/>A la nage<br/>
				<input type="radio" name="locomotion" value="Traineau" <?php echo( isset($_POST['locomotion']) && $locomotion == "Traineau" ? 'checked="checked"' : '') ?>/>En traineau<br/>
			</td>
		</tr>
		<tr>
	...
	<div style="font-weight: bold;">
		<br/>
		Vous êtes disciple de Jésus-Christ et vous avez à cœur de gagner des âmes à Christ, alors inscrivez-vous pour l'évangélisation à Strasbourg via ce formulaire.
		<br/>
		&nbsp;
	</div>
